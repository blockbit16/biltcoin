...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected BILT and zBILT supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zBILTsupply']:
                assert_equal(node['zBILTsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check BILT and zBILT supply at the beginning
        # ------------------------------------------
        # zBILT supply: 2 coins for each denomination
        ...
        # BILT supply: block rewards minus burned fees for minting
        ...
        # Verify that BILT and zBILT supplies were properly updated after the spends and reorgs
        self.log.info("Check BILT and zBILT supply...")
