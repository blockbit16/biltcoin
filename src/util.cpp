...
const char * const PIVX_CONF_FILENAME = "bilt.conf";
const char * const PIVX_PID_FILENAME = "bilt.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\bilt
// Windows >= Vista: C:\Users\Username\AppData\Roaming\bilt
// Mac: ~/Library/Application Support/bilt
// Unix: ~/.bilt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bilt";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\biltParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\biltParams
    // Mac: ~/Library/Application Support/biltParams
    // Unix: ~/.bilt-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BiltParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "BiltParams";
#else
    // Unix
    return pathRet / ".bilt-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty bilt.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override bilt.conf
